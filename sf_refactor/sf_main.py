...
def main(cfg: DictConfig):
    ...
    if cfg.model.which_circuit == "new":
        ...
        phi_wr = [[prog.params(f"phi_{w}_{r}") for r in range(reuploads_per_wire)] for w in range(cfg.model.wires)]
        pt_wr  = [[prog.params(f"pt_{w}_{r}")  for r in range(reuploads_per_wire)] for w in range(cfg.model.wires)]
        ...
        weights = {
            ...
            **{f'squeeze_mag_{w}': squeeze_mag[w] for w in range(cfg.model.wires)},
            **{f'squeeze_phase_{w}': squeeze_phase[w] for w in range(cfg.model.wires)},
            # Flatten per-wire reupload symbols
            **{f'eta_{w}_{r}': eta_wr[w][r] for w in range(cfg.model.wires) for r in range(reuploads_per_wire)},
            **{f'phi_{w}_{r}': phi_wr[w][r] for w in range(cfg.model.wires) for r in range(reuploads_per_wire)},
            **{f'pt_{w}_{r}':  pt_wr[w][r]  for w in range(cfg.model.wires) for r in range(reuploads_per_wire)},
            **{f"cx_theta_{a}_{b}": cx_theta[(a,b)] for (a,b) in cx_pairs},
        }
        prog = reuploading_circuit(prog, cfg.model.wires, weights, reuploads_per_wire)
    else:
        weights = {
            's_scale': s_scale,
            **{f'disp_mag_{w}': disp_mag[w] for w in range(cfg.model.wires)},
    ...
    def make_args(jet_batch, jet_pt_batch):
        ...
        if cfg.model.which_circuit == "multiuploading":
            ...
            for w in range(cfg.model.wires):
                ...
                for r in range(reuploads_per_wire):
                    if squeeze:
                        d[f"eta_{w}_{r}"] = tf.squeeze(eta_val)
                        d[f"phi_{w}_{r}"] = tf.squeeze(phi_val)
                        d[f"pt_{w}_{r}"]  = tf.squeeze(pt_val)
                    else:
                        d[f"eta_{w}_{r}"] = eta_val
                        d[f"phi_{w}_{r}"] = phi_val
                        d[f"pt_{w}_{r}"]  = pt_val
        else:
        ...
        if cfg.model.which_circuit in ("new", "multiuploading", "reuploading"):
            for (a, b) in cx_pairs:
    ...
    if not cfg.runtime.cli_test:
        ...
        # Add cx_theta parameters if using circuits with trainable CX entanglers
        if cfg.model.which_circuit in ("new", "multiuploading", "reuploading"):
            for (a, b) in cx_pairs:

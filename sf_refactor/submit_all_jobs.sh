...
echo "Found $total_jobs job files to submit"
echo ""

# Ask user if they want to submit all or by type
...
if [[ $submit_all =~ ^[Yy]$ ]]; then
    ...
    echo "1. Submit GPU jobs only (faster, shorter walltime)"
    echo "2. Submit CPU jobs only (slower, longer walltime)" 
...
echo "Monitor job output in: /rds/general/user/lr1424/home/1P1Qm_SF/sf_refactor/sf_main_logs/"
